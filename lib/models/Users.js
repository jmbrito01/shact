...
Meteor.users.helpers({
    ...
    countFotosPrivadas:function(){
        return Fotos.find({userId:this._id,tipo:TIPO_PRIVADO}).count();   
    },
    pendentes:function(){
        return Conexoes.find({'userIds.1':this._id, status: CONEXAO_PENDENTE});        
    },
    conexoes:function(){
        return Conexoes.find({'userIds':this._id, status: CONEXAO_ACEITA});          
    },
    conexoesCount: function(){
        return Conexoes.find({'userIds':this._id, status: CONEXAO_ACEITA}).count();            
    },
    existeConexao:function(userId){
        if (userId==Meteor.userId()) return false;
        
        var c =Conexoes.findOne({userIds: {$all :[this._id, userId]}});
        return (c)?c:false;
    },
});
